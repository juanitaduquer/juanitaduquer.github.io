...
This is a compilation of some useful \Magma~tricks that I have learned through the years. Thank you to all the wonderful people who have shared their knowledge with me:
...
\href{https://c-keyes.github.io}{Chris Keyes},
...
\href{https://janeshi99.github.io}{Jane Shi},
...
\item In your home directory you can create a \verb|file.Magmarc|, if does not exist, to have commands to run on start of \Magma.  To find the file, you can go to your home directory and press Command + Shift + . (period).
...
\item To unpack a tuple, you can do \verb|a, b, c := Explode([2,3,7]);|. 
...
\verb|ListSignatures(ModFrmHilElt : Isa := false, Search := "Arguments");|.  Moreover, you can also just look for functions where your type is an argument or a return values (very useful when you try to find a function producing the type that another function needs…):
...
\item To print the $p$-Adic expansion of numbers, define yout pAdic field \verb|Zp| and do 

\verb|Zp`SeriesPrinting := true;|.
...
\item To set up ssh without a password, use something like \href{https://www.strongdm.com/blog/ssh-passwordless-login}{this} link. Do not forget to change your configuration file to avoid asking for a password when you ssh into your server. For example, add the following lines to your \verb|~/.ssh/config| file:
\begin{verbatim}
Host yourServerName
...
\item When you want a function to call itself, defining it as \verb|TheName := function()| does not work.  Try \verb|function TheName()| instead.
